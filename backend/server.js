 ...
 * Description: Generates the initial FutureMe profile response from the future self.
 */
app.post("/api/generate-futureme", async (req, res) => {
  try {
    const { name, age, goal, struggle, oneYearVision, userTimeline, tone } = req.body;

    // Support both userTimeline and oneYearVision for compatibility
 ...
 * Description: Interactive chat with the generated FutureMe profile.
 */
app.post("/api/chat-futureme", async (req, res) => {
  try {
    ...
    if (!userProfile || !question) {
      return res.status(400).json({
        success: false,
        error: "User profile and question are required to establish quantum connection."
      });
    }
...
// Start Server conditionally (only if run directly, not imported in serverless env)
if (require.main === module) {
  app.listen(PORT, () => {
    console.log(`==================================================`);
    console.log(`FutureMe Backend Server successfully initialized.`);
